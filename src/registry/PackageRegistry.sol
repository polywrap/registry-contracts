...
abstract contract PackageRegistry is Ownable, IPackageRegistry {
  ...
  function transferOrganizationOwnership(bytes32 _organizationId, address _newOwner) 
    ...
    bytes32 _packageId,
    address _newOwner
  ) public virtual override onlyOrganizationOwner(packages[_packageId].organizationId) {
  ...
  function _setPackageOwner(
	...
	function organizationOwner(bytes32 _organizationId) public virtual override view returns (address) {
    return organizations[_organizationId].owner;
	}
	
	function organizationExists(bytes32 _organizationId) public virtual override view returns (bool) {
    return organizations[_organizationId].exists;
	}

	function organization(bytes32 _organizationId) public virtual override view returns (bool exists, address owner) {
  ...
  function packageExists(bytes32 _packageId) public virtual override view returns (bool) {
    return packages[_packageId].exists;
  }

  function packageOwner(bytes32 _packageId) public virtual override view returns (address) {
    return packages[_packageId].owner;
  }

  function packageOrganizationId(bytes32 _packageId) public virtual override view returns (bytes32) {
    return packages[_packageId].organizationId;
  }

  function package(bytes32 _packageId) 
    public virtual override view returns (bool exists, address owner, bytes32 organizationId) {
    return (
      packages[_packageId].exists,
      packages[_packageId].owner,
      packages[_packageId].organizationId
    );
  }
